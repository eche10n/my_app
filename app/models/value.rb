class Value < ApplicationRecord
  ...
  scope :find_user_valued, -> (current_user_id, image_id){
    select('value').where(user_id:current_user_id, image_id:image_id)
  }

  def self.calc_average_value(image_id)
    values_arr = self.where(image_id: image_id).pluck(:value)
    logger.info "Values array = #{values_arr}"
    values_sum = values_arr.inject(:+)
    logger.info "Values sum = #{values_sum}"
